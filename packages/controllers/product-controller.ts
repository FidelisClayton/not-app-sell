...
import { CustomerDocument } from "@shared/models";
import { CreateProductServerSchema } from "@shared/models/product-model";
...
const handleGetUnowned = async (
  ...
  try {
    const products = await ProductRepository.getUnowned(customer._id, appId);
